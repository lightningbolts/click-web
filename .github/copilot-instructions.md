...
- Maps & visualizations: `maplibre-gl` is used in `components/dashboard/ConnectionMap.tsx` (Carto tiles + beacon APIs) and `components/landing/playground/PlaygroundMap.tsx` (client-only inline style, no network). Keep those imports in client components. Never use `mapStyleForTheme` on the landing playground.
