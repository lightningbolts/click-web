...
- **Visual identity:** `lib/ui/generateCardVisual.ts` must stay aligned with KMP `ui/theme/CardVisual.kt` — same FNV-1a seeding, the same weighted `CardHueFamily` table (purple, blue, teal, coral, gold, magenta, green; purple is the heaviest bucket at 5/16), the same WCAG 4.5:1 scrim search, and the same pin shapes. Paint generated surfaces through `components/ui/CardVisualSurface.tsx` (`CardVisualHero`) or `lib/ui/cardVisualPattern.ts` (`cardVisualStyle` / `cardVisualStyleCss`) — the pattern layer is part of the contract, not decoration web may skip. Seed with the **raw entity id**, never a list-key prefix. Beacon photos are **optional** on both platforms and upload via `POST /api/beacons/image` (2 MB, unencrypted).
