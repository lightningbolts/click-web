...
- **Landing playground map** is client-only (`playgroundMapStyle.ts` inline GeoJSON). Do not point it at Carto/`mapStyleForTheme` or `/api/map/*`. Theme changes must retint paint properties in place — remounting MapLibre resets the camera and jumps the page.
