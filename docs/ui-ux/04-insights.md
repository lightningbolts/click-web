...
- Same [`ProductAppShell`](../../components/shell/ProductAppShell.tsx): identity row (logo + account) then a wrapping Insights tool nav, both in [`PAGE_COLUMN_CLASS`](../../lib/shell/pageColumn.ts) so cards line up with the bar. Animated mobile drawer on small screens (not a desktop sidebar). Opaque `surface`, 1px `border-hard`; no blur orbs or glass icon wells.
