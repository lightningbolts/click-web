...
- **Root `/` must SSR marketing for anonymous requests.** `app/page.tsx` is a Server Component that chooses `LandingPage` vs `HomeAuthenticated` from the cookie session. `/events` and `/e/*` are public. `/insights` stays gated. Do not reintroduce a client `useAuth().loading` → `LoadingScreen` gate on the anonymous path — that hid all marketing copy from crawlers (issue #58 §1.4).
- **Landing playground map** lazy-loads MapLibre and uses Carto tiles from the **browser → cartocdn.com**, never `/api/map/*` or the Worker (`transformRequest` drops same-origin). Clamp to Seattle bounds. Theme changes must `setStyle` in place — remounting MapLibre resets the camera and jumps the page.
...
- **Maestro web CI** (`.github/workflows/e2e.yml`) installs Chrome 151 to match Maestro's bundled ChromeDriver, points `/usr/bin/google-chrome` at that binary with `--no-sandbox` **prepended**, and runs Maestro headed under Xvfb (not `--headless`). Local `npm run test:e2e` stays headed. Workspace `url:` is a literal origin — never `${BASE_URL}` — so Chromium does not open `data:`. Smoke asserts use unsplit AX-tree text and heading `id`s; styled spans (e.g. "About Click") are not a single Maestro text node.
- **Cloudflare worker build** is `npm run build:worker`. GitHub `build-worker` proves `.open-next/worker.js`. Keep `npm run build` as `next build` — OpenNext and Workers Builds (`npm run deploy`) call it; pointing it at OpenNext when `WORKERS_CI=1` recurses until timeout. Preview deploys (`npx wrangler versions upload`) use wrangler `build.command` to generate the worker if it is missing.
