...
- **No `.env.local` is committed.** Copy `.env.local.example` → `.env.local`. GitHub CI runs `scripts/materialize-ci-env.sh` (`.env.example` + repo secrets overlay) before `next build` and `npm run build:worker`. Production Worker secrets stay on the Cloudflare dashboard.
...
- **Maestro web CI** (`.github/workflows/e2e.yml`) installs Chrome 131 (Maestro Selenium CDP), points `/usr/bin/google-chrome` at that binary with `--no-sandbox` **prepended**, and runs `maestro test --headless`. Local `npm run test:e2e` stays headed. Workspace `url:` is a literal origin — never `${BASE_URL}` — so Chromium does not open `data:`.
- **Cloudflare worker build** is `npm run build:worker` (`opennextjs-cloudflare build`). GitHub CI job `build-worker` and `wrangler.jsonc` `build.command` must produce `.open-next/worker.js` — `npm run build` alone is not a Worker deploy.
