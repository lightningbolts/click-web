...
| Route | Who | Notes |
|-------|-----|--------|
| `/events` | Public + signed-in | Shared `EventPageShell` (`max-w-6xl`) and the same horizontal Navbar as the rest of the site. Signed-in visitors see **Your events** (`DashboardEventsModule`) then **Discover** (public list). Anonymous visitors see the public list only. Search + date/going/host chips, featured soonest event, then Today / This week / Upcoming. Cards use `CardVisualHero` (no date-rail). Route navigations show [`EventRouteLoading`](../../components/events/EventRouteLoading.tsx) skeletons (`app/events/loading.tsx`, `app/e/[beaconId]/loading.tsx`, `app/events/new/loading.tsx`); the ready page uses `EventPageEnter` for a short, no-delay 12px fade/up after the route skeleton resolves; reduced-motion visitors get an immediate render. Hosted cards: top-right icon actions for Edit / Host settings. Only `event_visibility = public` **and** `visibility_audience = everyone`. Unlisted / invite-only stay off this feed. |
